...
cd backend && python scripts/cross_sectional_alpha_model.py --walk-forward --borrow-sweep      # short-leg stock-borrow fee sensitivity (breakeven ~500bps/yr vs realistic ~50-150bps → survives)
cd backend && python scripts/cross_sectional_alpha_model.py --save-model                        # persist h=21 model → data/cross_sectional_{model,features}.json for the live SHADOW scorer
cd backend && python scripts/cross_sectional_alpha_model.py --save-model --horizon 63           # persist h=63 quarterly model → *_h63.json (parallel live shadow, deployed 2026-06-11)
cd backend && python scripts/cross_sectional_alpha_model.py --nested-horizon                     # honest horizon validation (per-fold ex-ante selection; 2026-06-11: h=63 unanimous 12/12, nested net 0.576 CI [0.22,0.91], haircut 0.000)
cd backend && python scripts/cross_sectional_alpha_model.py --walk-forward --placebo            # 3 pure-noise features → harness noise floor (~±0.1 net Sharpe); feature deltas inside the band are NOT signal
...
| Intraday floor | **40%** (RE-ENABLED 2026-06-15) | `delivery_gates.py:STYLE_CONF_FLOORS` (was 999/disabled; owner request. Momentum strategy → **exempt from the MR-setup gate**. Self-correcting safety rail `_intraday_safety_blocked()`: auto-blocks if live WR <40% over ≥30 resolved intraday BUYs, auto-recovers. Expect WR/Sharpe drag — monitor.) |
| min_confidence | **40%** | `config.py` (recalibrated 57→40 — honest scale after phantom-win correction) |
| Sector calibration | per-sector isotonic, top-priority curve | `calibration.py` (2026-06-15: confidence calibrated to each sector's empirical WR so the floor gates weak sectors **instead of hard blocks** — XLF/XLP/XLV/XLC map below 46 floor, XLK ~56% delivers. Sector derived from `SECTOR_MAP` (81% of rows have null `sector_etf`). Priority: sector→regime→global.) |
| Calibration deploy gate | **K-fold CV** Brier < 0.248 | `calibration.py:_cv_brier()` (2026-06-15: was a single temporal 80/20 split that a regime-shifted recent window false-rejected → calibration had been silently OFF/passthrough. CV Brier 0.244 < naive 0.25 → now deploys. Still rejects degenerate maps = zero-delivery protection intact.) |
...
| positionSizeScale | L1×…×L10, product clamped **[0.10, 3.00]** | `signal_engine.py` + `scanner.py` (L4=(conf−40)/14+0.5 clamped [0.5,1.5]; L7=score-band step 0.50–1.55×; L8=quality_score ≥43→1.30×/35–43→1.0×/<35→0.75× +0.06Sh; L9 bear dampener removed; **L10=§87 conviction tier (2026-06-10): prior-trading-day BUY existed → 1.3×, verified weighted A/B Sh 0.24→0.30, applied in `scanner._apply_l10_conviction_sizing()` which also enforces the global clamp**) |
| SELL delivery | **disabled** (long-only regime) | `delivery_gates.py` (2026-06-10: live SELLs 35.2% net WR, −1.00%/trade + conf-35 floor bypass; backtest §32 had already disabled SELLs) |
| BLOCKED_SECTORS | XLF/XLP/XLU/XLI — hard-blocked, **no dynamic unblock** | `delivery_gates.py` (2026-06-10: model-file unblock clause DELETED — it had silently unblocked XLF/XLI/XLP = 32% of delivered book at ≈−1.4%/trade; files quarantined to `data/quarantine/`; unblock now requires QENG-1c promotion) |
| EOD batch guard | price-validity, not age (DELIV-1) | `scanner.py` (skip iff price ≥ entry+0.5×ATR or ≤ stop; clean-sector stale deliveries earn +2.12%/trade → age is the wrong variable; skips persist to `signals.skip_reason`) |
...
| Macro regime HMM | `hmmlearn.GaussianHMM` (2-state, full cov, sticky prior) | `macro_regime.py` — replaced hand-rolled Baum-Welch/Viterbi 2026-06-09; same public contract (`get_macro_regime()`). **2026-06-15 tz fix:** SPY (Polygon, tz-aware) ∩ ^VIX (yfinance, tz-naive) returned empty → HMM always fell back to "unknown"; now `_to_tz_naive()` aligns indexes before intersect. |
| §86 XS shadow scan window | **2y** (not 1y) | `scanner.py` (2026-06-15 bugfix: `_price_features` needs ≥253 bars for 12-1 momentum; 1y≈251 → `score_batch()` returned `{}` every scan → §92 shadow accrued ZERO since 2026-06-09. Now 173/173 tickers score.) |
...
All §47–§83 implemented. **v10.10 (2026-06-15, live-quality session — tuning levers declared spent):** Headline finding from `gate_contribution_analysis.py` on 570 resolved: **live baseline WR 43.7%, and the edge is sector-concentrated — XLK Technology (54.4%, N=180) is the ONLY sector >50%** (XLY 45, XLC 44, XLV 41, XLI 36, XLF 34, XLP 30). This is a *tech mean-reversion* strategy diluted across sectors that don't work live. Response (owner: "don't block sectors, calibrate"): **sector-specific calibration** (`calibration.py`) — per-sector isotonic, top-priority curve, sector from `SECTOR_MAP`; XLF/XLP/XLV/XLC now calibrate below the 46 floor (gated naturally), XLK ~56% delivers. While building it, found **calibration had been silently OFF**: the self-gate's single temporal 80/20 split was false-rejecting a genuinely-skilled map (regime-shifted recent window) → switched to **K-fold CV gate** (CV Brier 0.244 < naive 0.25 → deploys; degenerate-map protection intact). Also: **intraday RE-ENABLED** (40% floor, MR-exempt, self-correcting safety rail); **§86/§92 XS shadow bugfix** (scan 1y→2y — it had accrued ZERO data since 2026-06-09); **9 tickers added** to live watchlist via `screen_universe_expansion.py` sweep (CBRE/NXPI/EL/TRV/IP/MCK/ROK/CMI/MET — but most are in non-tech/blocked sectors so few actually deliver; locked as `_EXPANSION_COHORT_20260615` forward set); **feed deliverability tagging** (full BUY book tagged + reason); **admin `alpha_guards`** observability; **UI honesty pass** (removed fabricated regime mock / `$`-as-`%` card bug / synthetic dashboard chart→real OHLCV + 1D/1W/1M/YTD/ALL filters / real sparklines / honest delivery-log + sector/calendar empty states); **HMM tz fix**. Lesson: the OHLCV/gate/calibration/universe tuning levers are now spent — calibration gating MORE signals *worsens* the signal-starvation blocking the calibration/§85-1/§92 audits (the quality-vs-volume tension). Next real edge = orthogonal data (paid) or a strategic narrowing to tech-MR; not more tuning. **v10.9 (2026-06-10, §87–§94 agenda + live delivery overhaul — see Stats.md §DELIV):** New canon (plain run; §94 per-sector hold-days now default): **N=217, WR=69.1%, +0.80%, Sh=0.24**, Lo CI [0.10, 0.37] ✅, MC P5=0.07 ✅ — but **Deflated Sharpe FAILS at the honest 744-trial count** (E[max]=0.25 > 0.24 ⚠): the IS lever is statistically spent; edge proof now lives in the post-fix forward window/OOS/orthogonal data, not more IS sweeps. §87 L10 conviction sizing verified (weighted A/B Sh 0.24→**0.30**, ΔN=0) and deployed live; §88 calm sleeve dead (0 trades in 23yr — MR triggers structurally never fire at VIX<20); §90 WATCH bench all rejected (0 trades); §89b factor attribution: alpha +0.87%/day (p=0.044), 95% idiosyncratic. **Live delivery overhaul (same day):** sector model-file unblock deleted (XLF/XLP/XLI were never actually blocked — 32% of delivered book at ≈−1.4%/trade), SELL delivery disabled, §55/§14 hard blocks removed from delivery (fresh A/B: §14 −0.06 Sh, harmful — **verdict RETRACTED 2026-06-12**: that A/B ran while a FRED `realtime_start` bug had emptied the panel; first honest measurement after the fix reads §14 at **+0.03 Sh, MaxDD −2.31%→−0.93%, 22 blocked**, consistent with the pre-bug +0.02 read — live re-instatement is an open QENG-1c decision, see LEARNINGS 2026-06-12), all `sector_rs` couplings decoupled, DELIV-1 price-validity EOD guard, `skip_reason` persisted. **Honest live re-baseline: clean book (May+ BUYs ex-blocked) = 57.8% net WR, +2.06%/trade net** — the live-vs-IS gap was mostly delivery; all pre-fix live audits are contaminated (81% null sector_rs + leaks). **v10.7 (2026-06-09, awesome-quant library audit):** Two hand-rolled algorithms replaced with battle-tested libraries (numpy-1.26 safe; +`statsmodels`/`hmmlearn`/`arch` to requirements). (1) **§63 cointegration ADF gate** — added the missing Engle-Granger step 2 (residual ADF stationarity test, p<0.10) to `technicals.py` (live, unconditional) and `backtest_technicals.py` (`_COINT_REQUIRE_STATIONARY` toggle). Was a correctness bug: §63 awarded +4pp on non-cointegrated (spurious-regression) pairs. Before/after on identical survivorship-corrected code: gate OFF (old) N=189/Sh=0.27 → gate ON (new) **N=205/Sh=0.23** (both clear Lo-CI + deflated-SR). The −0.04 IS Sharpe is the §76-Altman pattern again — the ungated edge was partly a spurious artifact acting as an incidental quality filter; gate kept for correctness/OOS-robustness. (2) **Macro regime HMM → `hmmlearn.GaussianHMM`** — replaced ~150 lines of hand-rolled Baum-Welch/forward-backward/Viterbi in `macro_regime.py`; identical public contract, fixed random_state removes label-switching risk in `hmm_bull_prob`/`hmm_trans_risk` meta-label features. Also wired `arch` GARCH(1,1) forward-vol as opt-in triple-barrier widths (`train_metalabel_model.py --barriers garch`; ablation ΔAUC≈0, default stays ATR). Research prototypes in `backend/scripts/research/` (skfolio HRP, alphalens IC — isolated env, numpy-2.x). **v10.6 (2026-06-09):** S&P 500 survivorship-bias correction enforced (point-in-time constituents from fja05680/sp500.git) — trims universe to historically-valid members; IS N=189, Sh=0.27 (higher quality than uncorrected v10.5). Also fixed a broken `--oos`/`--full-universe` path: `process_ticker` was refactored to return scored indicator frames instead of trades, but `run_oos_validation`/`run_full_universe_curation_bias` still unpacked the old tuple order and skipped `simulate_ticker` — they now call `simulate_ticker` on each frame. **v10.5 (2026-06-03):** §63 sector cointegration added to IS backtest scoring (rolling 252d Engle-Granger coint_z); §76 Altman removed from live engine (74% false-positive rate); dead gate cleanup complete; IS N=230, Sh=0.20. **v10.3 (2026-06-02, math-audit fixes):** ADX>35 ATR branch removed; Hurst window 63→64 bars; portfolio ann.Sharpe denominators fixed. **v10.2/v10.1:** ATR≤70 REVERTED, L9 removed, L8 quality_score recalibrated. Backtest canon (v10.7, 2026-06-09):

| Universe | N | WR | Avg Ret | Sharpe | MC P5 | 95% CI |
|---|---|---|---|---|---|---|
| **IS (v10.9 CANON, §94 sector holds default, 2026-06-10)** | **217** | **69.1%** | **+0.80%** | **0.24** | **0.07** ✅ | **[0.10, 0.37] ✅ (DSR ⚠ fails @744 trials)** |
| IS v10.9 + §87 L10 sizing (weighted, deployed live) | 217 | 70.7%w | +0.99%w | **0.30** | — | — |
...
**§86 update #2 (2026-06-11, free alt-data integration — RETRACTED/UNDER REVIEW):** A rigorous code review found two implementation defects that invalidate the first-pass headline numbers (h=21 net +0.308, h=63 net +0.769):
  * **Defect A: market-wide features were z-scored to death.** UMCSENT, NAAIM, and AAII are identical for every ticker on a date. `cross_sectional_zscore()` standardized per-date, giving std=0 → NaN → `fillna(0)`. The model only sees `*_z` columns, so these features were all-zero dead columns. The apparent +0.095 from `--naaim` was XGBoost `colsample_bytree=0.8` sampling noise, not signal. They are now passed through RAW as regime/context features.
  * **Defect B: FINRA SV and Wikipedia were merged same-day, creating ~1-day lookahead.** The per-trade PIT merge functions correctly lag +1 day, but `build_panel()` called `load_*_panel()` raw and used `merge_asof(..., direction='backward')` same-day. At mean IC ~0.003, a one-day optimistic shift on flow/attention features can be the entire effect. They are now shifted +1 day before the merge.
  * **Coverage caveats:** Wikipedia is a 20-mega-cap pilot (AAPL, MSFT, NVDA, TSLA, META, GOOGL, AMZN, JPM, …), 2015-07 → 2024-12, so ~480 names get zero and the last 1.5 years of the walk-forward have no wiki data. FINRA SV is consolidated NMS only, 2019-01 → 2024-12, not the §104-promised 2009+ per-venue history.
  * **Corrected single-split preview (h=21, 10bps one-way):** baseline net 0.369; `--finra-sv` net 0.414; `--wiki` net 0.451; `--naaim` net −0.182 (market-wide features now usable and harmful); `--naaim --wiki` net 0.111. Placebo noise columns (3 seeds) net 0.151–0.159.
  * **Corrected walk-forward (h=21, 10bps one-way):** baseline net 0.195; `--finra-sv` net 0.287; `--wiki` net 0.215; `--naaim` net 0.163; `--naaim --wiki` net −0.028; placebo (3 random-noise features, seed 1) net 0.310. The alt-data Δs are inside the placebo noise floor; none reliably beat the baseline after correcting the defects.
  * **Corrected walk-forward (h=63, 10bps one-way):** baseline (price only) net 0.616 (90% CI [+0.29, +0.94]); `--wiki` net 0.694 (90% CI [+0.31, +1.15]); placebo (3 random-noise features) net 0.705 / 0.716 / 0.707 across three seeds. The strong h=63 result is driven primarily by the **longer-horizon / lower-turnover cost structure**, not Wikipedia. Wiki adds only ~+0.08 net Sharpe, which is inside the placebo noise floor. Note: h=63 was selected as the max of the contaminated sweep, so even the clean baseline CI is **post-selected** and should be treated as an upper bound (~4 rebalances/year × 14 folds ≈ 56 independent decisions).
  * **Honest status:** No alt-data config has demonstrated a reproducible, properly-lagged, selection-adjusted effect clearly above the harness noise floor. The h=63 0.769 claim is withdrawn. Placebo tests and per-fold attribution by coverage epoch are still pending. See `docs/LEARNINGS.md §104–§110` for the full post-mortem.

**§86 update #3 (2026-06-11, nested-horizon validation + parallel h=63 shadow):** After the alt-data retraction (update #2), the HORIZON effect itself was put through honest validation: new `--nested-horizon` mode selects the horizon for each walk-forward fold using ONLY prior folds (grid {21,40,63}, burn-in 2, full universe, price features only). **h=63 won ex ante in all 12 eval folds (2014–2025)** → nested net Sharpe **+0.576 [90% CI +0.22, +0.91]**, selection haircut **0.000** (h=21 same folds: +0.419); corroborated by an independent single-split test (+0.494 OOS 2020–2026). Full h=63 WF track: net 0.616 [CI +0.29, +0.94], 10/14 folds positive, MaxDD −11.8%, **cost-robust to 40bps one-way (+0.481, where h=21 goes negative)**, borrow breakeven ≈700bps/yr vs ~50–150bps realistic GC. Caveat: grid descends from the contaminated sweep → validates 63-beats-21/40, not 63-optimal. **Deployed as a PARALLEL live shadow (2026-06-11, server restarted):** `--save-model --horizon 63` → `data/cross_sectional_{model,features}_h63.json`; `cross_sectional_shadow.score_batch_h63()`; `scan_all()` attaches `crossSectionalShadowPctH63` alongside the h=21 field. §92 promotion criteria stay tied to h=21 ONLY; the h=63 shadow never feeds sizing. Backtest default `HORIZON` stays 21.

**§86 borrow test + LIVE SHADOW deployment (2026-06-09):** The one cost an L/S backtest omits is stock-borrow on the short leg — tested via `--borrow-sweep`. h=21 net Sharpe survives to a **breakeven ~500bps/yr** borrow; realistic borrow for this S&P-500-large-cap universe (≥$5, ≥$10M/day → general collateral) is ~50–150bps, so the edge **survives** (net ~+0.28–0.31 at realistic borrow; ~+0.13 in a conservative 20bps-exec + 150bps-borrow combo). Verdict: promising-but-thin deploy *candidate*, not real capital yet (CI still grazes 0, h=21 vs live ~10d hold, not forward-validated). Wired to live in **SHADOW MODE ONLY**: `--save-model` persists the h=21 model to `data/cross_sectional_{model,features}.json`; `services/cross_sectional_shadow.py::score_batch()` ranks each scan batch; `signal_engine.scan_all()` attaches `crossSectionalShadowPct` + a rationale entry to each directional signal and logs it — **it does NOT change action/confidence/positionSizeScale** (collects forward data to validate before it ever moves a trade). The lighter active path later is a bottom-decile positionSizeScale haircut; do not enable until shadow data confirms the edge transfers to the live horizon/universe.
