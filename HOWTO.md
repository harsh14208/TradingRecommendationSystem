...
> **Version: v5.3** · Last updated: 2026-05-10
...
1. Go to `/signup`
2. **Step 1 — Choose plan**: Free (view only), Basic ($9.99/mo, Telegram + backtest), Pro ($19.99/mo, all features). Select one, click Continue.
...
> **Note:** `intraday` style was retired in v5.3. Historical validation (n=210) showed a 44.5% win rate — below break-even after costs. RSI/Bollinger oversold signals that previously fired as intraday now fire as `swing`, giving them the 3–7 day window where the model's 64.1% win rate actually exists.
...
> `intraday` style was retired in v5.3 (44.5% historical win rate). All previously-intraday setups are now classified as `swing` to give the entry signal a proper resolution window.
...
- Trained on resolved signals (`outcome_pct IS NOT NULL`) from the local database
- Extracts a 20-feature vector from each signal: confidence, sentiment, source flags, options/dark pool presence, style, stop/target distances, price magnitude, etc.
- Uses a temporal 70/30 train/test split (oldest 70% = train, newest 30% = OOS validation)
- Outputs a win probability; compares it to what the current confidence implies
- Adjusts confidence by ±25% (multiplicative, capped at 84% ceiling)
- Runs after Platt calibration — the last step before signal assembly
...
```bash
cd backend
pip install pytest pytest-asyncio pytest-cov pip-audit
pytest tests/ -v
```
...
Tests live in `backend/tests/`. Three test files currently:
- `test_imports_smoke.py` — import-only smoke tests (fast, no DB)
- `test_signal_engine_smoke_extended.py` — signal engine unit tests with mock data
- `test_signal_engine_confidence.py` — confidence scoring edge cases
