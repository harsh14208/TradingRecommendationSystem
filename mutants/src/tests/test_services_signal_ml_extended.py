...
def test_get_meta_model_file_absent(tmp_path):
    from services.signal_ml import get_meta_model
    import services.signal_ml as ml

    orig_dir = ml._DATA_DIR
    ml._DATA_DIR = tmp_path / "nodata"
    try:
...
def test_get_meta_model_load_success(tmp_path):
    from services.signal_ml import get_meta_model
    import services.signal_ml as ml

    fake_dir = tmp_path / "data"
    fake_dir.mkdir()
    model_file = fake_dir / "meta_label_model.json"
    model_file.write_text("{}")
...
def test_get_meta_model_cache_hit(tmp_path):
    from services.signal_ml import get_meta_model
    import services.signal_ml as ml
...
def test_get_meta_model_exception():
    ...
    with patch.object(ml._META_MODEL_FILE, "exists", return_value=True):
        with patch.object(ml._META_MODEL_FILE, "stat", side_effect=Exception("boom")):
            assert get_meta_model() is None
...
def test_get_model_oserror_on_stat():
    ...
    with patch.object(ml._MODEL_FILE, "exists", return_value=True):
        with patch.object(ml._MODEL_FILE, "stat", side_effect=OSError("denied")):
            result = get_model()
...
def test_get_entry_model_oserror_on_stat():
    ...
    with patch.object(ml._ENTRY_MODEL_FILE, "exists", return_value=True):
        with patch.object(ml._ENTRY_MODEL_FILE, "stat", side_effect=OSError("denied")):
...
def test_train_model_training_only():
    ...
    mock_model.feature_importances_ = [0.1] * 23
    mock_model.predict.return_value = [1] * 20
    mock_model.predict_proba.return_value = [[0.4, 0.6]] * 20

    np_mock = MagicMock()
    np_mock.array.return_value = "X_arr"
    np_mock.isnan.return_value = np_mock
    np_mock.mean.return_value = [0.0] * 23
    ...
    with patch("services.signal_ml._load_resolved_signals_sync", return_value=rows):
        with patch.dict(sys.modules, {"xgboost": xgb_mock, "numpy": np_mock, "sklearn.metrics": sklearn_mock}):
...
def test_train_model_deploys_first_run():
    ...
    mock_model.get_booster.return_value = mock_booster
    mock_model.feature_importances_ = [0.1] * 23
    mock_model.predict.return_value = [1] * 50
    mock_model.predict_proba.return_value = [[0.3, 0.7]] * 50

    np_mock = MagicMock()
    np_mock.array.side_effect = lambda x, dtype=None: x
    np_mock.isnan.return_value = np_mock
    np_mock.mean.return_value = [0.0] * 23
...
def test_train_challenger_model_single_class():
    ...
    mock_model = MagicMock()
    mock_model.feature_importances_ = [0.1] * 24
    mock_model.predict_proba.return_value = [[0.3, 0.7]] * 50
        ...
        with patch("services.signal_ml._load_resolved_signals_sync", return_value=rows):
            with patch.dict(sys.modules, {"xgboost": xgb_mock, "numpy": np_mock, "sklearn.metrics": sklearn_mock}):
                with patch.object(Path, "exists", return_value=True):
                    with patch("pathlib.Path.read_text", return_value='{"oos_auc": 0.65}'):
                        with patch("pathlib.Path.write_text"):
                            result = train_challenger_model()
...
async def test_load_resolved_signals_async():
    ...
    with patch("services.signal_ml.AsyncSessionLocal", return_value=mock_session_ctx):
