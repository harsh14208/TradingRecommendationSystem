...
3. For users with `auto_execute=True`, `broker_svc.execute_signal_for_user()` runs:
   ...
   - **AI pre-execution review** (`services/ai_evaluator.py`) — enabled by default. Set `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, or `KIMI_API_KEY` in `.env` and choose the provider with `AI_EVAL_PROVIDER`. The LLM reviews the signal after all engineering risk guards and can block a trade; the result is stored in `BrokerOrder.ai_eval_data`. If no key is configured, the gate logs a warning and lets the trade proceed.
