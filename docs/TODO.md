...
- [~] **R10-11: Frontend (8.8 → 10)** — FE-2 accessibility **DONE** (2026-06-12, 0 axe-core violations on /app) + Lighthouse perf budget **DONE** (2026-06-13): `.lighthouserc.js` tightened to error-level thresholds (FCP ≤2000 ms, LCP ≤3500 ms, CLS ≤0.05, TBT ≤200 ms, performance ≥0.70, accessibility ≥0.95, best-practices ≥0.95) and a `lighthouse` job added to CI with authenticated Puppeteer login. Assertions currently fail on /app due to large bundles and render-blocking resources; optimization needed before green. Remaining: typed API client + ≥1 golden-path ACT-6 E2E green in CI.
