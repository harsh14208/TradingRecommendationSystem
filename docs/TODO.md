...
### Scanner & Delivery
- [ ] **DISC-1. Alpaca WebSocket rapid reconnect loop** — stderr.log is 66 MB and ~95% Alpaca WS reconnect messages (`no frame in 15s` → reconnect every 20s). This drowns out real scanner logs and suggests the WS connection is unstable. Fix: add exponential backoff, or suppress reconnect logs after the first N per hour.
- [ ] **DISC-2. Scanner logs missing after 12:42 UTC** — No scanner delivery logs in stderr.log for the 13:35 / 13:55 / 14:16 scans despite signals being created in DB. Possible causes: log file handle lost, log rotation bug, or Alpaca WS flood starving the logger. Needs investigation.
- [ ] **DISC-3. `send_log` records wrong `chat_id`** — `_maybe_send` writes `settings.telegram_chat_id` to `send_log.chat_id` even when the signal was delivered via fan-out to a different user (e.g., user 7). The log says chat_id=8259206621 (owner) but the actual recipient was 8663290397. Misleading for delivery audits.
- [ ] **DISC-4. Timezone mismatch in cooldown / daily-cap queries** — `_maybe_send` computes `cutoff` and `_today_start` using `datetime.now(_ET)` (Eastern) but compares against `Signal.sent_at` which is stored in UTC. Raw naive comparison works for afternoon signals but can be wrong for morning signals (ET date ≠ UTC date). Fix: store or compare in a single timezone.

### Database & Schema
- [ ] **DISC-5. pytest polluting production DB** — pytest processes (PID 94347) were running and inserting test signals into the live PostgreSQL database. Signals with future timestamps (15:13 UTC while system time was 14:40 UTC) appeared in `signals`. Fix: ensure test suite uses isolated test DB (SQLite fixture) and never touches Postgres.
- [ ] **DISC-6. Alembic migration chain gap** — Migration `abc123def456` (add `raw_confidence`) exists on disk but was never applied. Alembic version jumped from `8dab442e6933` → `4a7f6b33eb49`, skipping the intermediate migration. The column was added manually later. Fix: verify migration chain integrity; stamp `abc123def456` into `alembic_version` history so downgrades don't break.
- [ ] **DISC-7. `signal_deliveries` missing `created_at`** — The table has no `created_at` column, making time-based queries awkward (had to use `sent_at` instead). Add the column with a migration.
- [ ] **DISC-8. `users.min_confidence_override` stale for user 3** — DB column shows `60`, but effective threshold is `45` from `notification_prefs` JSON. The column value is ignored in `_fanout_to_subscribers` when prefs exist. Either sync the values or drop the column if prefs are the source of truth.

### DevEx & Tooling
- [ ] **DISC-9. `ruff` missing from `.venv`** — Had to `pip install ruff` manually. Should be pinned in `requirements.txt` / `requirements-dev.txt` so every contributor gets it.
- [ ] **DISC-10. pre-commit hook pointed to dead Python path** — `.git/hooks/pre-commit` was using `backend/venv/bin/python3.14` which no longer exists. Reinstalling fixed it, but the hook should be resilient to venv moves (use `#!/usr/bin/env python3`).
...
- [ ] **§104. FINRA per-venue daily short-sale volume — extends the live short-volume gate from 2024+ to 2009+ (FREE, highest priority).** The live Polygon/Massive short-volume gate is historically unvalidatable (data starts ~2024-02). [FINRA publishes daily short-sale volume files](https://www.finra.org/finra-data/browse-catalog/short-sale-volume-data/daily-short-sale-volume-files) free: per-venue TRF/ADF files since **2009-08-03**, consolidated NMS since 2018-08-01 ([API available](https://developer.finra.org/docs/reg-sho-daily-short-sale-volume)). Published same evening (6pm ET) → next-day-usable, PIT-clean.
  ...
  - [ ] §104b. Ablation: short-volume-ratio (short/total, level + 5d delta) as entry-context columns; per-fold A/B as a sizing tilt. The §91 squeeze-fuel prior (live +2.42pp rising-SI spread) says elevated-and-falling short pressure at an oversold print = fuel.
  - [ ] §104c. If validated: swap the live gate's data source to FINRA-direct (removes the Massive dependency for this signal) and register the SPRT for forward confirmation.
- [ ] **§105. SEC fails-to-deliver panel — the only new source covering the FULL 23-year IS window (FREE).** [SEC publishes half-month FTD files](https://nasduck.substack.com/p/how-to-read-sec-ftd-and-not-fall) since 2004 (CSV; open-source parsers exist, e.g. juszhan/Fail-To-Deliver). Persistent FTDs = settlement stress = the literal mechanics of forced selling. ⚠ PIT discipline: published ~15–30d after the settlement date — as-of-join on **publication date**, not trade date.
  ...
  - [ ] §105b. Ablation: FTD percentile at entry (own-history rank) as sizing tilt; hypothesis from the mechanism: elevated FTDs at an oversold print = genuine dislocation (size up), not value trap. Full-window per-fold read — this is the rare new feature testable across GFC + COVID + 2022.
- [ ] **§106. NAAIM (2006+) + AAII (1987+) weekly positioning/sentiment — deep-history regime tilts (FREE).** [NAAIM Exposure Index](https://en.macromicro.me/charts/46198/naaim-exposure-index): actual manager equity exposure, weekly since 2006, −200..+200. [AAII bull/bear survey](https://www.aaii.com/sentimentsurvey/sent_results) weekly since 1987. Both classic contrarian washout markers — orthogonal to VIX level (positioning vs implied vol). ⚠ Lag to release day (Wed/Thu) in the join.
...
- [ ] **§107. GDELT historical news tone — first-ever backtest of the news-sentiment family (FREE, bounded pilot).** The live engine scores news sentiment on every signal but the family has NEVER been historically validated. [GDELT](https://www.gdeltproject.org/) is 100% free (raw files + BigQuery), archives to 1979, GKG 2.0 with per-org tone from 2015. Entity→ticker mapping is the hard part — pilot is bounded: 20 IS tickers × 2015+, company-name match in GKG organizations, daily tone z-score panel. Question: does negative-news-tone-at-oversold-entry predict the bounce (capitulation) or the knife (repricing)? Either answer recalibrates a live family that currently runs on faith.
- [ ] **§108. FINRA ATS dark-pool weekly per-ticker (2014+) — institutional participation context (FREE).** [Weekly per-security ATS volumes](https://www.finra.org/filing-reporting/otc-transparency) since Rule 4552 (Jan 2014), [file-download API](https://www.finra.org/sites/default/files/OTC-Transparency-Data-File-Download-API-v04.pdf). ⚠ 2-week publication lag (Tier 1) — a slow regime/quality feature, not an entry trigger: dark-pool share trend as institutional-participation context for L8-style quality sizing. 12 years backtestable; also replaces the live `dark_pool.py` real-time-only stubs with something validatable.
...
- [ ] **§110. OCC daily volume/OI by symbol — free per-stock put/call for the LIVE options family (live-only).** [OCC market-data reports](https://www.theocc.com/market-data/market-data-reports/volume-and-open-interest/daily-volume) publish daily volume + OI by symbol free. No deep per-symbol history → not backtestable, but it gives the live §49/§69–§72 options family an exchange-grade daily PCR/OI source independent of the yfinance chain (and a fallback when Polygon options 403s). Accumulate-forward from day 1 so a future validation window exists. Also folds into the §98 decision: if OCC daily + accumulated history suffices, the ORATS month may only be needed for the IV-rank (not flow) hypotheses.
