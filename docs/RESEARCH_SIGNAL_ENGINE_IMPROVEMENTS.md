...
quantile.  Isolating the MAX21 effect (no R7 DD-throttle) keeps **264 of 313 trades (−16%)**
and produces event-time Sharpe **3.75**, MaxDD **−7.15%**, CAGR +4.1% — a +0.47 Sharpe
improvement with a ~1.3pp MaxDD cut versus the unthrottled baseline.

When the R7 step-function DD-throttle is stacked on top of the 0.55 filter (the current
`--portfolio` default), the combined path is Sharpe **4.35**, MaxDD **−5.17%**, CAGR +4.2%.
**That 4.35 is the combined MAX21 + DD-throttle effect, not the isolated MAX21 lift.**

A threshold sensitivity on the isolated (no-throttle) run shows **0.55 is the sweet spot**:

| `--max21-filter` | Trades kept | ΔN vs baseline | Port Sharpe | Max DD | CAGR |
|:---|---:|---:|---:|---:|---:|
| baseline (off, no throttle) | 313 | — | 3.28 | −8.43% | +4.31% |
| 0.33 | 191 | −39% | 4.01 | −5.34% | +3.70% |
| 0.50 | 251 | −20% | 4.12 | −5.51% | +3.90% |
| **0.55** | **264** | **−16%** | **3.75** | **−7.15%** | **+4.10%** |
