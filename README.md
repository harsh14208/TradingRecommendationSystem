...
**Institutional-grade trading signals — 50+ independent indicators across 15 categories, real-time options sweeps, 13F institutional flow with QoQ trend tracking, structural market invariants, multi-user Telegram delivery, and a full subscription stack.**
...
- **Runs full subscription billing** via Stripe (Free / Basic $9.99 / Pro $19.99)
...
The engine uses a sigmoid capped at **84% maximum confidence** (hard ceiling):

```
confidence ≈ 40 + 44 × (1 − e^(−|score|/65)) + agreement_bonus
```

This ceiling is set by empirical calibration — signals above 84% historically yielded 50-67% actual win rates, a 15-38pp overconfidence gap. The `⚠ CALIB` badge fires when confidence exceeds historical win rate by >20pp.
...
| **Basic** | $9.99/mo | + Telegram delivery, backtest analytics, custom watchlist, signal history |
| **Pro** | $19.99/mo | + Paper trading, correlation matrix, predictive intervals, sector heatmap, calibration dashboard, simulated backtest |
