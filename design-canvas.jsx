// design-canvas.jsx — Figma-ish design canvas wrapper
// Save separately; not bundled into app.jsx.
// Usage: wrap DCArtboard children inside DCSection inside DesignCanvas.
// See comments in code for full API.
...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  return (
    <div ref={vpRef} style={{ height:'100vh', width:'100vw', background:DC.bg, overflow:'hidden', overscrollBehavior:'none', touchAction:'none', position:'relative', fontFamily:DC.font, boxSizing:'border-box', ...style }}>
  ...
  return (
    <div data-dc-section={sid} style={{ marginBottom:80, position:'relative' }}>
      <div style={{ padding:'0 60px 56px' }}>
        <DCEditable tag="div" value={sec.title ?? title} onChange={v => ctx && sid && ctx.patchSection(sid, { title: v })}
...
function DCPostIt({ children, top, left, right, bottom, rotate = -2, width = 180 }) {
  return (
    <div style={{ position:'absolute', top, left, right, bottom, width, background:DC.postitBg, padding:'14px 16px', fontFamily:'"Comic Sans MS","Marker Felt","Segoe Print",cursive', fontSize:14, lineHeight:1.4, color:DC.postitText, boxShadow:'0 2px 8px rgba(0,0,0,0.12)', transform:`rotate(${rotate}deg)`, zIndex:5 }}>{children}</div>
  );
}
