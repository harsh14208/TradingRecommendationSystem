...
async def get_economy_data() -> dict:
    ...
    if isinstance(yields_raw, list) and yields_raw:
        ...
        if y2  is not None: result["yield_2y"]  = round(float(y2),  3)
        if y10 is not None: result["yield_10y"] = round(float(y10), 3)
        if y30 is not None: result["yield_30y"] = round(float(y30), 3)
        if y2 and y10:
    ...
    if isinstance(inflation_raw, list) and inflation_raw:
        ...
        if cpi is not None: result["cpi_yoy"]  = round(float(cpi), 2)
        if pce is not None: result["pce_yoy"]  = round(float(pce), 2)
    ...
    if isinstance(labor_raw, list) and labor_raw:
        ...
        if ue  is not None: result["unemployment"]     = round(float(ue), 2)
        if nfp is not None: result["nfp_change_k"]     = round(float(nfp) / 1000, 1)
