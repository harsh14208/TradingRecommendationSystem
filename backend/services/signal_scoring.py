...
def score_moving_averages(
    ...
    if ema200 and sma200:
        ...
        if above_ema200 and above_sma200:
            ma_delta += 3
            rationale.append({"src": "Technical", "head": "Double Bullish: Above EMA200 & SMA200",
                "body": (f"Price ${price:.2f} is above both EMA(200) (${ema200:.2f}) and SMA(200) (${sma200:.2f}). "
                         "Institutional algorithms use EMA(200) as a trend filter — double confirmation reduces false signals."),
                "sentiment": "pos", "meta": f"EMA200=${ema200:.2f} SMA200=${sma200:.2f}"})
        elif not above_ema200 and not above_sma200:
            ma_delta -= 3
            rationale.append({"src": "Technical", "head": "Double Bearish: Below EMA200 & SMA200",
                "body": (f"Price ${price:.2f} is below both EMA(200) (${ema200:.2f}) and SMA(200) (${sma200:.2f}). "
                         "Confirmed long-term downtrend by both smoothing methods."),
                "sentiment": "neg", "meta": f"EMA200=${ema200:.2f} SMA200=${sma200:.2f}"})
