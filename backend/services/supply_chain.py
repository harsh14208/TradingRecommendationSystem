...
_BDI_BULL_TICKERS  = {"XLE", "XLB", "XLI", "CAT", "DE", "FCX", "SCCO", "UPS",
                       "EOG", "CVX", "XOM", "SLB", "AEM", "NEM", "COPX"}
...
async def _fetch_bdi() -> dict | None:
    ...
    try:
        ...
        if   chg_20d >  15: trend = "strong_bull"
        elif chg_20d >   5: trend = "bull"
        elif chg_20d < -15: trend = "strong_bear"
        elif chg_20d <  -5: trend = "bear"
        else:               trend = "neutral"
    ...
    if bdi:
        ...
        if trend in ("strong_bull", "bull"):
            ...
            for t in _BDI_BEAR_TICKERS:
                add(t, -pts * 0.5, f"BDI rising → import cost headwind for retailers")
        elif trend in ("strong_bear", "bear"):
            pts = min(8.0, abs(chg) * 0.4)
            for t in _BDI_BULL_TICKERS:
                add(t, -pts, f"BDI {chg:.0f}% (20d) — shipping demand contracting")
            for t in _BDI_BEAR_TICKERS:
                add(t, +pts * 0.3, f"BDI falling → lower import costs for retailers")
...
async def get_supply_chain_signals() -> dict:
    ...
    result = {
        ...
        "sources_available": [
            s for s, d in [("BDI", bdi), ("Brent Crude", energy), ("Cass Freight", freight)]
            if d is not None
        ],
        "cached_at": int(now),
    }
    ...
    _cache["ts"] = now
    log.info(f"[supply_chain] BDI={bdi['value'] if bdi else 'N/A'} "
...
_SUPPLIER_CUSTOMER_MAP: dict[str, list[str]] = {
    ...
    "SCCO": ["GM", "CAT", "DE", "HON"],
    # Retail supply: consumer discretionary → logistics
    "AMZN": ["UPS", "FDX"],
...
_PROPAGATION_DIRECTION: dict[str, int] = {
    "TSM": +1, "AMAT": +1, "KLAC": +1, "LRCX": +1,
    "MSFT": +1, "AMZN": +1, "GOOGL": +1,
    "XOM": -1, "CVX": -1,   # energy costs → headwind for transportation
    "FCX": -1, "SCCO": -1,  # materials costs → headwind for manufacturers
    "WMT": +1,
}
...
def get_supply_chain_propagation_score(
    ...
    for supplier, customers in _SUPPLIER_CUSTOMER_MAP.items():
        ...
        boost += capped
        reasons.append(
            f"{supplier} {s_action} ({s_conf:.0f}% conf) → "
            f"{'tailwind' if capped > 0 else 'headwind'} for {ticker}"
        )
