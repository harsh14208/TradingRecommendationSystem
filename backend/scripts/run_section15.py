"""
run_section15.py — Focused §15 research runner.

Skips §13 + §14 entirely. Loads full 68-ticker universe, applies extra
indicators, then runs §15 (sector-specific filter research):
  ...
  §15f  Best combined      — auto-selected winners applied together

Expected runtime: ~30 minutes.

Usage:
    cd backend
    python scripts/run_section15.py 2>&1 | tee /tmp/decomp_§15.log
"""
...
def main() -> None:
    print("# §15 Sector-Specific Filter Research\n")
    print(f"> Skipping §13-§14. Loading data then jumping straight to §15.")
    ...
    # ── Pre-score once; share pool for §15 ───────────────────────────────────
