...
def main() -> None:
    ...
    print("  Params    : thresh=35, ATR≥20, no §17f gates, no VIX floor")
    print("  vs §19    : thresh=38, ATR [20,70], ret_jump<-6%")
    ...
    if edge_real:
        ...
        print("    The §15f/§17f SECTOR-SPECIFIC threshold stack was the overfit source.")
        print("    The core MR edge is real. Recommendation: simplify _SECTOR_MR_CONFIG,")
        print("    rely on global ATR≥20 quality filter rather than per-sector VIX/thresh.")
    else:
        print(f"  ✗ BASE SIGNAL ALSO WEAK — only {n_passed}/{n_total} OOS windows pass.")
        print("    The core MR edge itself is weaker than in-sample (not just sector overfit).")
        print("    Possible causes: regime shift post-2016, or IS results were partially lucky.")
        print("    Recommendation: accept lower expected live Sharpe (~0.3-0.5 per trade).")
    ...
    for w, n19, wr19, avg19, ann19 in s19:
        ...
        ann20 = f"{r20['ann_sharpe']:.2f}" if r20 and not math.isnan(r20.get("ann_sharpe", float("nan"))) else "n/a"
        n20 = r20["n"] if r20 else 0
        delta = (
            r20["ann_sharpe"] - ann19 if r20 and not math.isnan(r20.get("ann_sharpe", float("nan"))) else float("nan")
        )
        d_str = f"{delta:+.2f}" if not math.isnan(delta) else "n/a"
        print(f"    {w}  §19 N={n19:>3} Ann={ann19:.2f}  →  §20 N={n20:>3} Ann={ann20}  (Δ={d_str})")
