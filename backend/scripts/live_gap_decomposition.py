...
async def main():
    async with AsyncSessionLocal() as db:
        res = await db.execute(
            select(
                ...
                Signal.sector_etf,
                Signal.session,
            ).where(Signal.outcome_pct.is_not(None), Signal.action.in_(["BUY", "SELL"]))
        )
        ...
        res2 = await db.execute(
            select(
                ...
                BrokerOrder.avg_fill_price,
                BrokerOrder.arrival_price,
                BrokerOrder.spread,
            ).where(BrokerOrder.avg_fill_price.is_not(None))
        )
        fills = pd.DataFrame(res2.all(), columns=["symbol", "filled_at", "avg_fill_price", "arrival_price", "spread"])
        fills = fills.copy()
    ...
    if signals["vix"].notna().sum() == 0:
        print("> VIX missing from all resolved signals — these rows pre-date the 2026-06-10")
        print("  scanner fix that populates extra_data. New signals going forward WILL")
    ...
    if len(reliable) > 0 and reliable["sent_at"].notna().sum() > 0:
        ...
        print("Reliable cohort (May+) latency:")
    ...
    if len(cata_returns) >= 10 and len(other_returns) >= 10:
        ...
        print(
            f"Catastrophic (11–12h ET): N={len(cata_returns)}, WR={(cata_returns > 0).mean() * 100:.1f}%, avg={cata_returns.mean():+.2f}%"
        )
        print(
            f"All other hours:          N={len(other_returns)}, WR={(other_returns > 0).mean() * 100:.1f}%, avg={other_returns.mean():+.2f}%"
        )
    ...
    if len(catastrophic) > 0:
        ...
        for h, row in catastrophic.iterrows():
            print(
                f"   Hour {h:02.0f}: WR={row['wr']:.1f}% (N={row['n']:.0f}), avg={row['avg']:+.2f}%, conf={row['conf_mean']:.1f}"
            )
    ...
    if len(catastrophic) > 0:
        for h in catastrophic.index:
            ...
            if len(low_conf) > 0:
                ...
                print(
                    f"  Low confidence (<60):  N={len(low_conf)}, WR={lc_wr:.1f}%, avg={low_conf['outcome_pct'].mean():+.2f}%"
                )
            if len(high_conf) > 0:
                hc_wr = (high_conf["outcome_pct"] > 0).mean() * 100
                print(
                    f"  High confidence (≥60): N={len(high_conf)}, WR={hc_wr:.1f}%, avg={high_conf['outcome_pct'].mean():+.2f}%"
                )
