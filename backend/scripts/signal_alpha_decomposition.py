...
import os, sys, warnings, math, itertools, time
...
from backtest_technicals import (
    TICKERS as _BASE_TICKERS, START, END, HOLD_DAYS,
    BUY_THRESH, BUY_THRESH_MAX, SELL_THRESH,
    FRICTION_PCT, POSITION_SIZE, REGIMES,
    MR_RSI_CEIL, MR_BB_CEIL, MR_IBS_CEIL, MR_VWAP_FLOOR,
    MR_GAP_FLOOR, MR_STREAK_CEIL,
    MOM_RSI_FLOOR, MOM_RSI_CEIL,
    simulate_ticker, stats, stats_weighted, fmt_sharpe, fmt_pf, print_table,
...
FAM_COLS = [
    "osc_f","mr_f",
    "trend_f","vol_f","ma_f","wk52_f",
    "cmf_f","donchian_f","hyg_f","pricestr_f",
    "rs_quality_f",
]
...
def compute_scores_masked(
    ...
    mr_raw += np.select(
        [bb_ok2&(bb_arr<0.05)&(rsi<35),   bb_ok2&(bb_arr<0.05)&(rsi>=35)&(rsi<45),  bb_ok2&(bb_arr<0.05),
         bb_ok2&(bb_arr>=0.05)&(bb_arr<0.15)&(rsi<35), bb_ok2&(bb_arr>=0.05)&(bb_arr<0.15)&(rsi>=35)&(rsi<45)],
        [18, 12, 6, 10, 5], default=0)
    mr_raw += np.select(
        [bb_ok2&(bb_arr>0.95)&(rsi>65),   bb_ok2&(bb_arr>0.95)&(rsi>55)&(rsi<=65),
         bb_ok2&(bb_arr>0.85)&(bb_arr<=0.95)&(rsi>65), bb_ok2&(bb_arr>0.85)&(bb_arr<=0.95)&(rsi>55)&(rsi<=65)],
        [-14, -8, -8, -4], default=0)
    ...
    b2u    = _v("vwap_band2_upper", np.inf);  b2u_ok = df["vwap_band2_upper"].notna().values if "vwap_band2_upper" in df.columns else np.zeros(n, bool)
    b2l    = _v("vwap_band2_lower", -np.inf); b2l_ok = df["vwap_band2_lower"].notna().values if "vwap_band2_lower" in df.columns else np.zeros(n, bool)
    ...
    mod_mr = (adx_val > 25) & ~strong; rng_mr = ~strong & ~mod_mr
    ...
    if return_families:
        return pd.DataFrame({
            "osc_f":osc_f,"mr_f":mr_f,
            "trend_f":trend_f,"vol_f":vol_f,"ma_f":ma_f,"wk52_f":wk52_f,
            "cmf_f":cmf_f,"donchian_f":don_f,"hyg_f":hyg_f,"pricestr_f":pstr_f,
            "rs_quality_f":rq_f,
            "score":score,
        }, index=df.index)
...
def run_ablation_suite(all_dfs, vix, spy_trend, stlfsi4, baseline_stats, baseline_trades):
    ...
    print(f"> Removed (v1-v10): mfi, candle, rsi_div, keltner, streak, pivot, super, hurst, gap,")
    print(f">   rsi_level, earn, redday, mom_decel, sector_rs, roc, rs, atr_reg — all ΔSharpe ≥ 0")
    print(f"> ROC/RS/ATR_REG removed in v10: penalise MR targets → less N AND worse Sharpe (v9 ablation)")
    ...
    abl_rows = [["**BASELINE** (10 families: OSC×1.0 + MR×0.5 + 8 filters, v10)", str(bs_n),
    ...
    print(f"\n---")
    print(f"*v12 · 11 families (OSC×1.0 + MR×0.5 + 8 filters + RS_QUALITY) · {len(all_dfs)}-ticker universe · ext-MR (gap+streak) · {len(REGIMES)} regimes · {datetime.today().strftime('%Y-%m-%d')}*")
...
def main() -> None:
    ...
    print(f"> v9 result: N=33, Sharpe=0.30 — ablation confirmed ROC/RS/ATR_REG all redundant.")
    print(f"> v10 fix: remove ROC, RS, ATR_REG — they penalise oversold MR targets, reducing N AND Sharpe.")
    print(f"> 17 confirmed redundant families removed total (mfi, candle, rsi_div, keltner, streak, pivot,")
    print(f">   super, hurst, gap, rsi_level, earn, redday, mom_decel, sector_rs, roc, rs, atr_reg).")
    ...
    print(f"Computing 11-family scores (OSC×1.0 + MR×0.5 + 8 filters + RS_QUALITY, extended MR gate: gap+streak)…", flush=True)
    ...
    print_table(["Version","Families","Config","N","WR","Avg Ret","Sharpe","MaxDD"], [
        ...
        [f"**v12**","11",f"{len(all_dfs)}-ticker, OSC×1.0+MR×0.5+8f+RS_QUAL",
    ...
    def _run_earn_test(blackout: int) -> tuple[dict, pd.DataFrame]:
        ...
        for ticker, df in all_dfs.items():
            t = simulate_ticker(
                ...
                mr_only=True, dual_gate=DUAL_GATE_ON,
    ...
    def _d(a, b, key): return (a.get(key) or 0.0) - (b.get(key) or 0.0)
    print("\n### Gate Comparison\n")
    print_table(
        ["Config", "N", "Win Rate", "Avg Ret", "Sharpe", "Max DD"],
        [
            ["blackout=5 (current, baseline)",
             str(s5["n"]), f"{s5['wr']:.1f}%", f"{s5['avg']:+.2f}%",
             fmt_sharpe(s5["sharpe"]), f"-{s5['max_dd']:.2f}%"],
            ["blackout=2 (proposed — hard-only)",
             str(s2["n"]), f"{s2['wr']:.1f}%", f"{s2['avg']:+.2f}%",
             fmt_sharpe(s2["sharpe"]), f"-{s2['max_dd']:.2f}%"],
    ...
    if not tdf2.empty and "days_to_earnings" in tdf2.columns:
        ...
        def _bucket(d):
            if d is None or (isinstance(d, float) and math.isnan(d)): return "No data"
            d = int(d)
            if d <= 2:  return "0-2d (hard blackout)"
            if d <= 7:  return "3-7d (pre-earnings)"
            if d <= 14: return "8-14d (early caution)"
            return "15+d (safe zone)"

        tdf2["earn_bucket"] = tdf2["days_to_earnings"].apply(_bucket)
        bkt_rows = []
        for bkt in ["0-2d (hard blackout)", "3-7d (pre-earnings)", "8-14d (early caution)", "15+d (safe zone)", "No data"]:
            sub = tdf2[tdf2["earn_bucket"] == bkt]
            if sub.empty: continue
            sv = stats(sub["net_pct"].tolist())
            bkt_rows.append([bkt, str(sv["n"]), f"{sv['wr']:.1f}%",
                             f"{sv['avg']:+.2f}%", fmt_pf(sv["pf"]), fmt_sharpe(sv["sharpe"])])
...
def run_sharpe_research(all_dfs, vix, spy_trend, stlfsi4):
    ...
    def _run_research(
        ...
        for ticker, df in all_dfs.items():
            ...
            t = simulate_ticker(
                ticker, df2, vix, spy_trend, stlfsi4,
                mr_only=True, dual_gate=False,
    ...
    for _i, hd in enumerate(_hold_vals, 1):
        ...
        hold_rows.append([f"hold={hd}d", str(sv["n"]),
                          f"{sv['wr']:.1f}%", f"{sv['avg']:+.2f}%",
                          fmt_sharpe(sv["sharpe"]), f"-{sv['max_dd']:.2f}%"])
    ...
    for _i, r in enumerate(_rsi_vals, 1):
        ...
        rsi_rows.append([f"RSI<{r}", str(sv["n"]),
                         f"{sv['wr']:.1f}%", f"{sv['avg']:+.2f}%",
                         fmt_sharpe(sv["sharpe"]), f"-{sv['max_dd']:.2f}%"])
    ...
    for _i, bt in enumerate(_thresh_vals, 1):
        ...
        thresh_rows.append([f"thresh≥{bt}", str(sv["n"]),
                            f"{sv['wr']:.1f}%", f"{sv['avg']:+.2f}%",
                            fmt_sharpe(sv["sharpe"]), f"-{sv['max_dd']:.2f}%"])
    ...
    combos_2d = [
        ...
        {"label": "hold=5 + RSI<35 + 1.0s/2.5t",  "hold_days": 5,  "mr_rsi_ceil": 35, "stop_mult": 1.0, "target_mult": 2.5},
        {"label": "hold=5 + RSI<38 + 1.0s/2.5t",  "hold_days": 5,  "mr_rsi_ceil": 38, "stop_mult": 1.0, "target_mult": 2.5},
        {"label": "hold=5 + RSI<35 + thresh≥44 + 1.0s/2.5t",
         "hold_days": 5, "mr_rsi_ceil": 35, "buy_thresh": 44, "stop_mult": 1.0, "target_mult": 2.5},
    ]
    ...
    for _i, cfg in enumerate(combos_2d, 1):
        ...
        flag = " ← **Sharpe>1**" if (sv.get("sharpe") or 0) >= 1.0 else (
               " ← Sharpe>0.7"  if (sv.get("sharpe") or 0) >= 0.7 else "")
        comb_rows.append([lbl + flag, str(sv["n"]),
                          f"{sv['wr']:.1f}%", f"{sv['avg']:+.2f}%",
                          fmt_sharpe(sv["sharpe"]), f"-{sv['max_dd']:.2f}%"])
...
def run_advanced_research(all_dfs, vix, spy_trend, stlfsi4):
    ...
    def _run12(
        ...
        trades_list = []
        fw = dict(BASE_WEIGHTS); fw["mr"] = mr_w
        for ticker, df in all_dfs.items():
            df2 = df.copy()
            df2["score"] = compute_scores_masked(df2, family_weights=fw)
            t = simulate_ticker(
    ...
    def _fmt12(label, sv, base_sh=0.32) -> list:
        ...
        return [label + flag, str(sv["n"]), f"{sv['wr']:.1f}%",
                f"{sv['avg']:+.2f}%", f"{fmt_sharpe(sv['sharpe'])} ({d_sh:+.2f})",
                f"-{sv['max_dd']:.2f}%", _ann(sv)]

    # ── Run MR=0.1 base once for reference ───────────────────────────────────
    base12, base12_tdf = _run12()
    base_sh12 = base12.get("sharpe") or 0.0
    print(f"Base (MR=0.1): N={base12['n']}, WR={base12['wr']:.1f}%, "
    ...
    if not base12_tdf.empty and "score" in base12_tdf.columns:
        ...
        print_table(["Config", "N", "WR", "Avg Ret", "Sharpe", "MaxDD", "Ann. Sharpe"], [
            ["Equal-weight (baseline)",
             str(base12["n"]), f"{base12['wr']:.1f}%", f"{base12['avg']:+.2f}%",
             fmt_sharpe(base12["sharpe"]), f"-{base12['max_dd']:.2f}%", _ann(base12)],
            ["Score-weighted (Kelly-style)",
             str(sw["n"]), f"{sw['wr']:.1f}%", f"{sw['avg']:+.2f}%",
             fmt_sharpe(sw["sharpe"]), f"-{sw['max_dd']:.2f}%",
    ...
    for _i, vm in enumerate(vix_vals[1:], 1):
        ...
        if (sv.get("sharpe") or 0) > best_vix_sh and sv["n"] >= 50:
            best_vix = vm; best_vix_sh = sv.get("sharpe") or 0.0
    ...
    best_count = 1; best_count_sh = base_sh12
    for _i, cnt in enumerate([2, 3], 1):
        ...
        if (sv.get("sharpe") or 0) > best_count_sh and sv["n"] >= 30:
            best_count = cnt; best_count_sh = sv.get("sharpe") or 0.0
    ...
    best_atr = None; best_atr_sh = base_sh12
    for _i, ar in enumerate(atr_vals[1:], 1):
        ...
        if (sv.get("sharpe") or 0) > best_atr_sh and sv["n"] >= 30:
            best_atr = ar; best_atr_sh = sv.get("sharpe") or 0.0
    ...
    combos_12 = [
        ...
        {"label": f"+ VIX≥{best_vix or 15:.0f} + consec",   "vix_min": best_vix or 15.0,
                                                              "require_consec": True},
        ...
        {"label": f"+ VIX≥{best_vix or 15:.0f} + MR≥2",    "vix_min": best_vix or 15.0,
                                                              "require_mr_count": 2},
    ]
...
def _pool_sim_ticker(args: tuple) -> "pd.DataFrame":
    ...
    return simulate_ticker(
        ticker, df, _pool_g_vix, _pool_g_spy, _pool_g_fsi,
        mr_only=True, dual_gate=False,
...
def _run_par(
    ...
    args = [(t, atr_min, vix_min, hold_days, thresh, atr_max, ret_jump, entry_delay, ibs_streak)
...
def run_atr_research(all_dfs, vix, spy_trend, stlfsi4,
                     spy_closes: "pd.Series | None" = None,
                     pool=None, pre_dfs: "dict | None" = None):
    ...
    def _ann(sv):
        ...
        if n < 5 or sh == 0: return "—"
        return f"{sh * math.sqrt(n / 20):.2f}"

    def _fmt13(label, sv, base_sh=0.38):
        sh = sv.get("sharpe") or 0.0; d = sh - base_sh
        flag = " ← **ANN≥1**" if _ann(sv) != "—" and float(_ann(sv)) >= 1.0 else ""
        return [label + flag, str(sv["n"]), f"{sv['wr']:.1f}%",
                f"{sv['avg']:+.2f}%", f"{fmt_sharpe(sv['sharpe'])} ({d:+.2f})",
                f"-{sv['max_dd']:.2f}%", _ann(sv)]
    ...
    with _managed_pool(pool, N_WORKERS, _pool_init,
                       (pre_dfs, vix, spy_trend, stlfsi4)) as _p:
        ...
        print(f"N={base13['n']}, WR={base13['wr']:.1f}%, "
              f"Sharpe={fmt_sharpe(base13['sharpe'])}, Ann={_ann(base13)}\n")
    ...
    def _fmt13_plain(label, sv, base_sh=0.0):
        sh = sv.get("sharpe") or 0.0; d = sh - base_sh
        return [label, str(sv["n"]), f"{sv['wr']:.1f}%",
                f"{sv['avg']:+.2f}%", f"{fmt_sharpe(sv['sharpe'])} ({d:+.2f})",
                f"-{sv['max_dd']:.2f}%", _ann(sv)]
    ...
    if sh_beta > base_sh13 + 0.01:
        print(f"> Per-trade Sharpe lift: {sh_beta - base_sh13:+.2f}  — "
              f"{'breaks 0.50 ceiling' if sh_beta >= 0.50 else 'still below 0.50 ceiling — alt-data or options needed'}")
    else:
        print(f"> Beta-hedge negligible or negative ({sh_beta:.2f} vs {base_sh13:.2f}). "
...
def run_tech_optimization(all_dfs, vix, spy_trend, stlfsi4,
                          pool=None, pre_dfs: "dict | None" = None):
    ...
    with _managed_pool(pool, N_WORKERS, _pool_init,
        ...
        def _run14(tech_hold=10, tech_thresh=40, atr_min=20.0):
            ...
            sv_t, tdf_t = _run_par(_p, tech_list,  atr_min=atr_min,
                                   hold_days=tech_hold, thresh=tech_thresh)
...
def run_sector_research(all_dfs, vix, spy_trend, stlfsi4,
                        pool=None, pre_dfs: "dict | None" = None):
    ...
    sector_lists: dict[str, list] = {
        name: [t for t in tset if t in pre_dfs]
        for name, tset in SECTOR_MAP.items()
    }
    ...
    def _row(label, sv):
        return [label, str(sv["n"]), f"{sv['wr']:.1f}%",
                f"{sv['avg']:+.2f}%", fmt_sharpe(sv.get("sharpe") or 0),
                f"-{sv['max_dd']:.2f}%", _ann_fmt(sv)]

    def _row_d(label, sv, base_sh=0.0):
        sh = sv.get("sharpe") or 0.0
        flag = " ← **ANN≥1**" if _ann_val(sv) >= 1.0 else ""
        return [label + flag, str(sv["n"]), f"{sv['wr']:.1f}%",
                f"{sv['avg']:+.2f}%", f"{fmt_sharpe(sh)} ({sh - base_sh:+.2f})",
                f"-{sv['max_dd']:.2f}%", _ann_fmt(sv)]
    ...
    with _managed_pool(pool, N_WORKERS, _pool_init,
        ...
        for sector in STRONG:
            ...
            if not sl:
                best_hold[sector] = 10; continue
            print(f"\n**{sector}** ({len(sl)} tickers):\n")
            rows = []
            best_ann = -1.0; best_h = 10
            for h in HOLD_VALS:
        ...
        for sector in STRONG:
            sl = sector_lists[sector]
            if not sl:
                best_vix[sector] = None; continue
            print(f"\n**{sector}** ({len(sl)} tickers):\n")
            rows = []
            best_ann = -1.0; best_vm: "float | None" = None
            for vm in VIX_VALS:
                sv, _ = _r(sl, vix_min=vm)
                lbl = f"No VIX floor (baseline)" if vm is None else f"VIX ≥ {vm:.0f}"
                rows.append(_row(lbl, sv))
                a = _ann_val(sv)
                if a > best_ann:
                    best_ann = a; best_vm = vm
        ...
        for sector in STRONG:
            sl = sector_lists[sector]
            if not sl:
                best_thresh[sector] = 40; continue
            print(f"\n**{sector}** ({len(sl)} tickers):\n")
            rows = []
            best_ann = -1.0; best_th = 40
...
def run_full_sector_research(all_dfs, vix, spy_trend, stlfsi4,
                             pool=None, pre_dfs: "dict | None" = None):
    ...
    SECTOR_MAP: dict[str, set] = {
        ...
        "Financials":   {"JPM","WFC","BAC","GS","V","MA","BLK","SCHW","CME","SPGI","MCO","ICE","MSCI","FIS","FISV"},
        "Consumer":     {"HD","F","LOW","TJX","ROST","LULU","MAR","HLT","RCL","CHTR","GM","COST","SBUX","TGT","PYPL","DHI","LEN"},
    ...
    def _row(sector, sv):
        return [sector, str(sv["n"]), f"{sv['wr']:.1f}%",
                f"{sv['avg']:+.2f}%", fmt_sharpe(sv.get("sharpe") or 0),
                f"-{sv['max_dd']:.2f}%", _ann_fmt(sv)]
    ...
    with _managed_pool(pool, N_WORKERS, _pool_init,
        ...
        for s in ALL_SECTORS:
            vl = f"≥{best_vix.get(s, None):.0f}" if best_vix.get(s) is not None else "none"
            print(f"  {s}: VIX {vl}")
        ...
        for sect in ALL_SECTORS:
            sl = sector_lists[sect]
            if not sl:
                best_atr[sect] = 20.0; continue
            best_ann = -1.0; best_at = 20.0
            row_parts = [sect]
            for at in ATR_VALS:
        ...
        def _row_d16(label, sv):
            sh = sv.get("sharpe") or 0.0; d = sh - base_sh16
            flag = " ← **ANN≥1**" if _ann_val(sv) >= 1.0 else ""
            return [label + flag, str(sv["n"]), f"{sv['wr']:.1f}%",
                    f"{sv['avg']:+.2f}%", f"{fmt_sharpe(sh)} ({d:+.2f})",
                    f"-{sv['max_dd']:.2f}%", _ann_fmt(sv)]
        ...
        for sect in ALL_SECTORS:
            ...
            kw = {}
            h = best_hold.get(sect, 10)
            if h != 10: kw["hold_days"] = h
            vm = best_vix.get(sect)
            if vm is not None: kw["vix_min"] = vm
            th = best_thresh.get(sect, 40)
            if th != 40: kw["thresh"] = th
            ba = best_atr.get(sect, 20.0)
            if ba != 20.0: kw["atr_min"] = ba
            sv, _ = _r(sl, **kw)
...
def run_gate_research_v17(all_dfs, vix, spy_trend, stlfsi4,
                          pool=None, pre_dfs: "dict | None" = None) -> None:
    ...
    with _managed_pool(pool, N_WORKERS, _pool_init,
                       (pre_dfs or {}, vix, spy_trend, stlfsi4)) as _p:
        ...
        for ceil_v in [90, 80, 70]:
            ...
            trades_c = []
            for _sv, _tl, _kw in [(_sv_t, TECH, {"hold_days":5,"vix_min":13.0,"thresh":40,"atr_max":float(ceil_v)}),
                                   (_sv_f, FIN,  {"hold_days":7,"vix_min":15.0,"thresh":42,"atr_min":30.0,"atr_max":float(ceil_v)}),
                                   (_sv_c, CONS, {"hold_days":10,"vix_min":13.0,"thresh":38,"atr_max":float(ceil_v)})]:
                _, _tdf = _r(_tl, **_kw)
                if not _tdf.empty: trades_c.append(_tdf)
            if trades_c:
        ...
        best_ceil = min([90, 80, 70], key=lambda v: -_ann_val(
            stats(pd.concat([r for _, r in [
            ...
            ] if not r.empty], ignore_index=True)["net_pct"].tolist()) if True else 0
        ))
        ...
        for thresh_j in [-8.0, -6.0, -5.0]:
            trades_j = []
            for _tl, _kw in [(TECH, {"hold_days":5,"vix_min":13.0,"thresh":40,"ret_jump":thresh_j}),
                ...
                _, _tdf = _r(_tl, **_kw)
                if not _tdf.empty: trades_j.append(_tdf)
            if trades_j:
        ...
        trades_d = []
        for _tl, _kw in [(TECH, {"hold_days":5,"vix_min":13.0,"thresh":40,"entry_delay":True}),
            ...
            _, _tdf = _r(_tl, **_kw)
            if not _tdf.empty: trades_d.append(_tdf)
        if trades_d:
        ...
        for _tl, _kw in [
            ...
            _, _tdf = _r(_tl, **_kw)
            if not _tdf.empty: trades_all.append(_tdf)
        if trades_all:
        ...
        for streak_n in [3, 5, 7]:
            ...
            for _tl, _kw in [
                ...
                _, _tdf = _r(_tl, **_kw)
                if not _tdf.empty: trades_ibs.append(_tdf)
            if trades_ibs:
                _tdf_ibs = pd.concat(trades_ibs, ignore_index=True)
                _sv_ibs  = stats(_tdf_ibs["net_pct"].tolist())
            else:
                _sv_ibs = dict(_EMPTY_STATS)
            ibs_rows.append(_row(f"IBS sole-trigger needs ≥{streak_n}d below SMA20", _sv_ibs, _ann_val(_sv_ibs), sv_base))
        ...
        for _tl, _kw in [
            ...
            (FIN,  {"hold_days":7, "vix_min":15.0, "thresh":42, "atr_min":30.0, "atr_max":70.0, "ret_jump":-6.0, "ibs_streak":5}),
            (CONS, {"hold_days":10,"vix_min":13.0, "thresh":38, "atr_max":70.0, "ret_jump":-6.0, "ibs_streak":5}),
        ]:
            _, _tdf = _r(_tl, **_kw)
            if not _tdf.empty: trades_full.append(_tdf)
        if trades_full:
