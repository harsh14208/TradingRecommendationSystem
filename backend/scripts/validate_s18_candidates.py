...
def main():
    ...
    with Pool(min(5, len(CANDIDATES))) as p:
        dl_results = p.map(process_ticker, [(t, vix, spy_trend, stlfsi4, True) for t in CANDIDATES])
    ...
    def _run(label, ceil=None, jump=None):
        ...
        for t, df in pre_dfs.items():
            ...
            if ceil is not None:
                kw["atr_pct_rank_max_override"] = ceil
            if jump is not None:
                kw["ret_jump_filter_override"] = jump
    ...
    b_pass = [
        t for t, sv in base.items() if (sv.get("wr") or 0) >= 55 and (sv.get("sharpe") or 0) >= 0.35 and sv["n"] >= 5
    ]
    f_pass = [
        t for t, sv in full.items() if (sv.get("wr") or 0) >= 55 and (sv.get("sharpe") or 0) >= 0.35 and sv["n"] >= 3
    ]
    ...
    if all_pass:
        for t in sorted(all_pass):
            ...
            f = full.get(t, {})
            b_wr = b.get("wr") or 0
            b_sh = b.get("sharpe") or 0
            verdict = "ADD" if b_wr >= 65 and b_sh >= 0.50 else "CAUTION"
            print(
                f"    {t}: §15f N={b.get('n', 0)} WR={b_wr:.0f}% Sh={b_sh:.2f}"
        ...
        for t in sorted(pre_dfs):
            ...
            f = full.get(t, {})
            print(
                f"    {t}: §15f N={b.get('n', 0)} WR={b.get('wr') or 0:.0f}% Sh={b.get('sharpe') or 0:.2f}"
                f"  |  §17f N={f.get('n', 0)}"
            )
