...
from models import ResearchExperiment, FeatureSnapshot, ModelRegistry, ActionAuditLog
...
async def test_feature_store_snapshots():
    ...
    async with AsyncSessionLocal() as db:
        ...
        snap = await save_feature_snapshot(
            db=db, ticker="TEST_TICKER", ts=ts, features=features, provider="polygon", signal_policy_version="v2.1"
        )
        await db.commit()
...
async def test_promote_model_checklist():
    ...
    async with AsyncSessionLocal() as db:
        ...
        audit_res = await db.execute(
            ...
            .where(ActionAuditLog.action == "promote_model")
...
async def test_tca_calculations_and_capacity():
    ...
    async with AsyncSessionLocal() as db:
        ...
        broker_data = {"filled_qty": 100.0, "qty": 100.0, "filled_avg_price": 150.50, "status": "filled"}
...
async def test_portfolio_hrp_allocations():
    ...
    cov = np.array([[0.04, 0.038, 0.0], [0.038, 0.04, 0.0], [0.0, 0.0, 0.01]])
...
async def test_alpha_sleeves_logic():
    ...
    assert isinstance(
        residuals, dict
    )  # returns empty or populated dict gracefully depending on market data availability
