...
def test_score_mda_delta_clamped():
    ...
    # Extreme negative
    current = (
        "risk risk risk adverse adverse adverse uncertainty uncertainty "
        "challenges headwind difficult deteriorate decline impair "
        "volatility volatility risk risk risk adverse"
    ) * 10
...
# ── has_active_buyback tests ──────────────────────────────────────────────────


@pytest.mark.asyncio
async def test_has_active_buyback_cache_hit():
    import services.edgar as ed
    import time

    ed._buyback_cache["AAPL"] = (True, time.time())
    from services.edgar import has_active_buyback

    result = await has_active_buyback("AAPL")
    assert result is True
    del ed._buyback_cache["AAPL"]


@pytest.mark.asyncio
async def test_has_active_buyback_no_cik():
    from services.edgar import has_active_buyback

    with patch("services.edgar._get_cik", new_callable=AsyncMock, return_value=None):
        result = await has_active_buyback("UNKNOWN_TICKER_XYZ")
