...
def _best_outcome(r: Signal):
    ...
    return (
        ...
        if r.outcome_3d is not None
        else r.outcome_1d
        if r.outcome_1d is not None
        else r.outcome_14d
    )
...
async def backtest_stats(
    ...
    tiers = [
        ("<50%", lambda c: c < 50),
        ("50–65%", lambda c: 50 <= c < 65),
        ("65–80%", lambda c: 65 <= c < 80),
        ("80%+", lambda c: c >= 80),
    ]
    by_confidence = []
    ...
    def risk_metrics(returns: list[float]) -> dict:
        ...
        return {"sharpe": sharpe, "max_drawdown": round(-max_dd, 2), "calmar": calmar, "ann_return": ann_return}
...
async def backtest_oos(
    ...
    oos_variance = sum((r - oos_mean) ** 2 for r in oos_returns) / (n_oos - 1) if n_oos > 1 else 0.0
...
async def update_notes(
    signal_id: int, body: _NotesIn, db: AsyncSession = Depends(get_db), _user: User = Depends(get_current_user)
):
    sig = (await db.execute(select(Signal).where(Signal.id == signal_id))).scalar_one_or_none()
    if not sig:
        raise HTTPException(404, "Signal not found")
...
async def track_record(
    ...
    rows = (await db.execute(select(Signal).where(*filters).order_by(desc(Signal.outcome_at)))).scalars().all()
...
async def predictive_confidence(
    ...
    h7_data = next((h for h in horizons if h["horizon"] == "7d"), None) or next(
        (h for h in horizons if h["horizon"] == "3d"), None
    )
            ...
            insights.append(
                {
                    ...
                    "body": (
                        ...
                        + (
                            "Asymmetry favours taking this trade."
                            if ratio >= 1.5
                            else "Risk/reward is roughly balanced."
                            if ratio >= 0.9
        ...
        try:
            ...
            if df is not None and not df.empty and best.created_at:
                ...
                if not future.empty:
                    ...
                    best_path = [
                        round((float(row["Close"]) - base_price) / base_price * 100, 2) for _, row in future.iterrows()
                    ]
        except Exception:
            pass
        ...
        insights.append(
            {
                ...
                + (
                    "This tier has been reliable."
                    if tier_wr >= 60
                    else "This tier has been mixed — consider reducing position size."
                    if tier_wr < 50
            ...
            insights.append(
                {
                    ...
                    + (
                        "Momentum is building — signal quality may be rising."
                        if delta > 0
    ...
    return {
        ...
        "message": (
            f"Based on {n} similar {action} signals"
            if n >= 5
...
async def backtest_horizons(
    ...
    if end_date:
        try:
            ...
            pass
    rows = (await db.execute(select(Signal).where(Signal.is_sent == True, *date_filters))).scalars().all()
...
async def signal_correlation(db: AsyncSession = Depends(get_db), _user: User = Depends(get_current_user)):
    ...
    if _cached is not None:
        return _cached
    rows = (
        (await db.execute(select(Signal).where(Signal.is_sent == True).where(Signal.sources.isnot(None))))
        .scalars()
        .all()
    )
    ...
    for (a, b), count in sorted(co_count.items(), key=lambda x: -x[1]):
        total_resolved = sum(
            1 for r in rows if a in (r.sources or []) and b in (r.sources or []) and r.outcome_pct is not None
        )
...
async def backfill_outcomes(db: AsyncSession = Depends(get_db), _user: User = Depends(get_current_user)):
    ...
    to_fill = [
        s
        for s in rows
        if s.outcome_1d is None or s.outcome_3d is None or s.outcome_pct is None or s.outcome_14d is None
    ]
    ...
    for ticker, sigs in by_ticker.items():
        try:
            ...
            for sig in sigs:
                ...
                def close_at(offset, _i=sig_idx, _td=trading_days):
                    idx = _i + offset
                    return _td[idx][1] if idx < len(_td) else None
...
async def execution_confirm(
    ...
    delivered_to = (
        (await db.execute(select(SignalDelivery.user_id).where(SignalDelivery.signal_id == sig.id))).scalars().all()
    )
