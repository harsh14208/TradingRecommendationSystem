...
async def create_checkout(
    ...
    if not user.stripe_customer_id:
        customer = st.Customer.create(email=user.email, name=user.full_name or user.email,
...
async def _handle_checkout_completed(session: dict, db: AsyncSession):
    ...
    try:
        ...
        user.subscription_period_end = datetime.fromtimestamp(sub["current_period_end"], tz=_tz.utc).replace(tzinfo=None)
        await db.commit()
...
async def _handle_subscription_updated(sub: dict, db: AsyncSession):
    ...
    sub_status = "active" if stripe_status in ("active", "trialing") else (
        "past_due" if stripe_status == "past_due" else "inactive"
    )
