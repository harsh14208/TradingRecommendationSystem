"""
Social OAuth router — Google only.
All providers share the same redirect→callback→one-time-code pattern:
  1. GET /api/auth/google               → redirect to consent screen
  2. GET /api/auth/google/callback      → exchange code, find/create user, issue JWT
