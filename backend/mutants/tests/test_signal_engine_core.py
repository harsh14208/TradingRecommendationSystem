...
def test_assemble_signal_risk_free_rate_dampener():
    ...
    with patch.object(_se, "datetime", wraps=_se.datetime) as _mock_dt:
        ...
        res = _assemble_signal(
            ticker="AAPL",
            info={"company": "Apple"},
            tech={"price": 100.0, "atr": 1.0, "rsi": 38.0, "ibs": 0.10},  # rsi<42 + ibs<0.15 satisfies MR gate; target 100+3*1=103
            score=50.0,  # BUY
            rationale=[],
            sources=set(),
            _force_hold=False,
...
def _mr_buy_kwargs(**overrides):
    ...
    defaults = dict(
        ticker="NVDA",
        info={"company": "NVIDIA"},
        tech={"price": 100.0, "atr": 2.0, "rsi": 38.0, "ibs": 0.10, "volume": 5_000_000, "avg_volume": 4_000_000},
        score=45.0,
        rationale=[],
        sources=set(),
        _force_hold=False,
